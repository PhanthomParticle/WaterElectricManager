<?php
	...
	//计算总页数
	$sql="SELECT DISTINCT uid FROM deposit WHERE ispay=0";
...
<head>
	...
	<link rel="stylesheet" type="text/css" href="css/nopay.css">
...
<body>
	...
	<ul id="list">
		...
		<?php
			$sql="SELECT DISTINCT uid FROM deposit WHERE ispay=0 LIMIT ".($page-1)*$pageSize.",".$pageSize;
			...
			for($i=0; $i<count($uid); ++$i){
				...
				echo "<li>";
					...
					echo "<a href=\"../upload/deposit.php?id=".$datas[$i]["uid"]."&url=../check/nopay.php&page=".$page."\" class=\"detail\">详细记录</a>";
				echo "</li>";
				$no++;
			}
	...
	<?php
		...
		echo "<a href=\"nopay.php?type=".$_GET["type"]."&value=".$_GET["value"]."&page=".($page-1)."\">上一页</a>";
		$pageStart=($col-1)*$colSize+1;
		$pageEnd=$col*$colSize;
		for($i=$pageStart; $i<=$pageEnd; ++$i){
			if($i==$page){
				echo "<a href=\"nopay.php?type=".$_GET["type"]."&value=".$_GET["value"]."&page=".$i."\" class=\"activep\">".$i."</a>";
			}else{
				echo "<a href=\"nopay.php?type=".$_GET["type"]."&value=".$_GET["value"]."&page=".$i."\">".$i."</a>";
			}
		}
		echo "<a href=\"nopay.php?type=".$_GET["type"]."&value=".$_GET["value"]."&page=".($page+1)."\">下一页</a>";
		echo "<input type=\"text\" name=\"topage\" id=\"topage\">";
		echo "<span>/".$pageCount."</span>";
		echo "<a href=\"nopay.php?type=".$_GET["type"]."&value=".$_GET["value"]."\" id=\"start\">跳转</a>";
