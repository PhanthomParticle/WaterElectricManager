<?php
	...
	$sql1="SELECT DISTINCT evalue.uid FROM evalue LEFT JOIN user ON user.uid=evalue.uid WHERE evalue.state=0 AND user.type=1";
	$sql2="SELECT DISTINCT wvalue.uid FROM wvalue LEFT JOIN user ON user.uid=wvalue.uid WHERE wvalue.state=0 AND user.type=1";
	$count1=count($databases->query($sql1)->fetchAll());
	$count2=count($databases->query($sql2)->fetchAll());
	
	if($count1>$count2){
		$count=$count1;
		$sql="SELECT DISTINCT evalue.uid FROM evalue LEFT JOIN user ON user.uid=evalue.uid WHERE evalue.state=0 AND user.type=1 ORDER BY evalue.time DESC LIMIT ".($page-1)*$pageSize.",".$pageSize;
	}else{
		$count=$count2;
		$sql="SELECT DISTINCT wvalue.uid FROM wvalue LEFT JOIN user ON user.uid=wvalue.uid WHERE wvalue.state=0 AND user.type=1 ORDER BY wvalue.time DESC LIMIT ".($page-1)*$pageSize.",".$pageSize;
	}
	if(($count%$pageSize)==0){
...
<body>
	...
	<?php
		...
		echo "<a href=\"wage.php?type=".$_GET["type"]."&value=".$_GET["value"]."&page=".($page-1)."\">上一页</a>";
		$pageStart=($col-1)*$colSize+1;
		$pageEnd=$col*$colSize;
		for($i=$pageStart; $i<=$pageEnd; ++$i){
			if($i==$page){
				echo "<a href=\"wage.php?type=".$_GET["type"]."&value=".$_GET["value"]."&page=".$i."\" class=\"activep\">".$i."</a>";
			}else{
				echo "<a href=\"wage.php?type=".$_GET["type"]."&value=".$_GET["value"]."&page=".$i."\">".$i."</a>";
			}
		}
		echo "<a href=\"wage.php?type=".$_GET["type"]."&value=".$_GET["value"]."&page=".($page+1)."\">下一页</a>";
		echo "<input type=\"text\" name=\"topage\" id=\"topage\">";
		echo "<span>/".$pageCount."</span>";
		echo "<a href=\"wage.php?type=".$_GET["type"]."&value=".$_GET["value"]."\" id=\"start\">跳转</a>";
